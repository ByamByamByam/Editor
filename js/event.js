...
function addBgmToSelectedRange() {
    ...
    if (
        ...
        alert(
            "BGM 주소는 YouTube 또는 SoundCloud 링크만 사용할 수 있습니다."
        );
        return;
    }
        ...
        chapter.events.filter(
            eventRange => {
                ...
                */
                ...
                /*
                    기존 구간이 새 구간 안에서 시작하면
                    새 이벤트가 우선하므로 기존 구간은 삭제한다.
                */
                return false;
...
async function playPreviewBgm(eventRange) {
    ...
    if (
        ...
        previewBgmPlayer.removeAttribute(
            "src"
        );
...
function updatePreviewSound() {
    ...
    /*
        같은 메시지가 계속 화면 중심에 있는 동안에는
        효과음을 반복 재생하지 않는다.
    */
    if (
        activeMessageId ===
        lastActiveSoundMessageId
    ) {
        return;
    }
...
async function playPreviewSound(soundEvent) {
    ...
    if (
        ...
        previewSoundPlayer.removeAttribute(
            "src"
        );
